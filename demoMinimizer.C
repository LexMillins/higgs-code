...
int main(int argc, const char *argv[]) {
       ...
       for (int i = 1 ;  i < argc ; i++) {
          ...
          if (arg == "-v") {
             ...
             std::cout << "" << std::endl;
             std::cout << "       -n <algorithm> : use given algorithm (possible names: simplex, minimize, scan, fumili)" << std::endl;
             std::cout << "       -v    : set minimul verbose mode to show final result" << std::endl;
             std::cout << "       -vv   : set medium verbose mode: show function value and edm at each minimization step" << std::endl;
